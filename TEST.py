...
async def get_match_info(date, state: FSMContext):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url=base_url, headers=headers, timeout=200) as resp:
            if resp.status == 200:
                result_data = []
                soup = BeautifulSoup(await asyncio.create_task(resp.text()), 'lxml')
                h = soup.find_all('h')
                print("Погнали")
                for game in h:
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('date_'), state=Time.date)
async def process_callback_date(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await asyncio.create_task(get_match_info(date, state))
...
if __name__ == '__main__':
    while True:
        try:
            updates = bot.get_updates(timeout=1200)
